...
public class EventOutBound {
    ...
    public void publish(Object event) throws IOException {
        BasicEvent basicEvent = (com.example.domain.events.BasicEvent) event;
        Headers headers = this.genHeader(basicEvent);
        Message message = NatsMessage.builder()
                .subject(basicEvent.getSubject())
                .headers(headers)
                .data(objectMapper.writeValueAsString(basicEvent), StandardCharsets.UTF_8)
                .build();
        JetStream jetStream = natsStream.getJetStream();
        CompletableFuture<PublishAck> future = jetStream.publishAsync(message);
        future.thenAccept((publishAck) -> {
            if (publishAck.hasError()) {
                ...
                log.info("發送事件已接收 {}", event);
            }
        });

    }

    private Headers genHeader(BasicEvent basicEvent) {
        Headers headers = new Headers();
        headers.add(DEFAULT_ATTR_PREFIX + _ID, basicEvent.getEventID());
        headers.add(DEFAULT_ATTR_PREFIX + _SOURCE, "//projects/accounts");
        headers.add(DEFAULT_ATTR_PREFIX + _SPECVERSION, "1.0");
        headers.add(DEFAULT_ATTR_PREFIX + _TYPE, basicEvent.getClass().getName());
        headers.add(DEFAULT_ATTR_PREFIX + _TIME, basicEvent.getTime().toString());
        headers.add(DEFAULT_ATTR_PREFIX + _DATACONTENTTYPE, "application/json");
        headers.add(DEFAULT_ATTR_PREFIX + _SUBJECT, basicEvent.getSubject());
        return headers;
    }
